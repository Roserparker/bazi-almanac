...
- `js/ziwei.js` —（2026-07-06）**紫微斗数引擎**：全书通行安星诀（闰月归本月、庚干「阳武阴同」、身主「子火午铃」）；`buildFromBirth/flowSiHua(流曜四化)/sanFangSiZheng`；**流层推宫 `flowLayers`**（流年=太岁宫、流月=斗君诀顺行、流日=流月宫起初一顺行；干支从农历侧：岁首换年/五虎遁月干/日柱）+ **`dayAdvice` 当日建议**（流日入宫×主星宜忌 STAR_ADVICE×四化落宫+臣曰）；星曜五行色表 + 十四主星两面性贴士 + 十二宫白话。**与 iztro 交叉验证 10 组生辰全合**（iztro 仅 devDependency，不进运行时）。`window.Ziwei`。
...
- `js/ui/ziwei.js` — 紫微星盘卡（**互动版，与万年历同步**）：日期条（跟随 state.sel，可翻日/回今天）、流层 chips 切换三层四化排布、宫格「年月日」落宫章、**点宫位出详批浮层**（palacePopHTML，app.js 委托）、「紫微观今日」建议块；万年历选定日面板有「紫微此日」速记行（almanac.js）。未录生辰为 CTA。
...
- ✅（2026-07-08）**分页式视图路由**：`#/board` hash 路由（app.js setView + body[data-view] + `.view[data-view]` CSS），六页=今日/万年历/八字盘(含录入)/紫微盘/BTC观象/学堂(含易学)，旧锚点自动映射；微互动一套（换页浮入/导航鎏金墨线/浮层弹入/宫格爻行九宫错落入场/指数数字滚动 `F.animateScores`，全部豁免 reduced-motion）；BTC 页六爻奇门折叠为「标题即结论」。
- ✅（2026-07-08）**紫微深化**：**大限**（起限=局数、阳男阴女顺/阴男阳女逆，与 iztro decadal 对拍全合；大限四化取限宫干）；宫位详批 v2（16 辅星两面注补齐、无主星教怎么借对宫、三方四正教怎么会商、大限/流年/流月/流日逐层视角分析）；**三方四正 SVG 连线**（焦点宫随流日、点宫换焦点）；**流曜轨迹**（换日旧宫残影渐隐）；建议区运限视角行。
- ✅（2026-07-08）**顶部信息架构**：导航自明分组（今日·万年历｜八字盘·紫微盘｜BTC观象｜学堂·易学）；「本站导览」四贴片卡（可收起记 localStorage `huaji.guide`）；锚点 smooth-scroll + `scroll-margin-top:54px` 修吸顶遮挡；未录生辰时 #personal 渲染 CTA（不再空白）；#intake 上移到紫微之后。**观象手记云端 routine 已全自动**（GitHub App 授权仓库写权限后打通；模型 claude-opus-4-8，每日 13:40 UTC；绿勾≠推送成功，验证看仓库有无「note: 观象手记」提交）。
- ✅（2026-07-07）**化机指数 v3 紧凑视图**：默认只显 总分行（分+档+一句建议）→ **化机分维**（财运/事业/情感/出行·搬家/学养，`Daily.dayDims`：总分为底向中回拉，按财官印五行能量、流日十神、夫妻宫合冲、驿马红鸾、**当日黄历宜忌白名单**微调）→ **宜/缓事项行**（黄历白话化 FOLK_CN 白名单 × 个人分维过滤，殡葬类词不进白名单）→ 合成明细全部折叠 `<details>`。仪表盘加分维小贴片；观象台 BTC 用 `compact:false` 保持全展开。
